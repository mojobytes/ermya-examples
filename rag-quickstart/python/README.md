...
into Ermya → demo search. When a `vls` config block is present, the same
...
`ermya_config.json` accepts an optional `vls` block:
...
The tests mock the Ermya SDK and the embedding HTTP calls, so they run without
a live Ermya server or network access.

## Layout

| File                        | Responsibility                                  |
|-----------------------------|-------------------------------------------------|
| `config_loader.py`          | Locate + parse `ermya_config.json` or defaults |
...
| `ermya_client_factory.py` | Build the Ermya SDK client from config        |
